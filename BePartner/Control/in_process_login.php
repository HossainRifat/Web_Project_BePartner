            ...
            if($row["Status"] == "Activate"){
                $_SESSION["email"] = $row["email"];
                    ...
                    if(isset($_POST["remember"])){
                                //session_start();
                                    setcookie("autologin",$in->encryption($_SESSION["email"]),time()+2*24*60*60,"/","");
                                }
                                header("location:../view/in_home.php");
                                //header("location:../control/in_process_cookie.php");
                                // setcookie("autologin",$_POST["email"],time()+2*24*60*60);
                                setcookie("log",$in->encryption($_SESSION["email"]),time()+2*24*60*60,"/","");
            }
            else{
                echo "Waiting for admin aproval.";
            }
